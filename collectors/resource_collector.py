        ...
        try:
            ...
            if existing_resource:
                if existing_resource["resource_version"] != item.metadata.resource_version:
                    # Use DeepDiff to get a serializable diff
                    difference = DeepDiff(existing_resource["data"], resource_dict, ignore_order=True).to_dict()
            ...
            logger.error(f"Failed to process resource {item.metadata.name} in {namespace}: {e}", exc_info=True)
    ...
    for cluster in CLUSTERS:
        ...
        for res_type in filter(lambda r: not r["namespaced"], RESOURCE_TYPES):
            try:
                ...
                logger.error(f"API Error fetching {res_type['name']} from {cluster_name}: {e.reason}", exc_info=True)
            except Exception as e:
